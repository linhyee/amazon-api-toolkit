...
function usage() {
    ...
    $usage = [
        ...
        "\t -t <type> report type\r\n",
        "\t -c <maxcount> max count\r\n",
        "\t --id=<id> report request id\r\n",
        "\t --ack=<act> true|false\r\n",
        "\t --afd=<afd> available from date\r\n",
        "\t --atd=<atd> available to date\r\n",
    ];
    print implode('', $usage);
    exit(0);
}
...
if (isset($type) && $type) {
    foreach (explode(',', $type) as $key => $val) {
        $param['ReportTypeList.Type.'.($key+1)] = $val;
    }
}
...
$rs = get_var($sxe, 'GetReportListResult');
if ($rs) {
    $info = get_var($rs, 'ReportInfo');
    if ($info) {
        foreach ($info as $val) {
